...
/********************************
      FUNCIONES PRIVADAS
********************************/
void Image::Allocate(int nrows, int ncols, img::byte * buffer){
    rows = nrows;
    cols = ncols;

    img = new img::byte * [rows];

    if (buffer != 0)
        img[0] = buffer;
    else
        img[0] = new img::byte [rows * cols];
...
void Image::Initialize (int nrows, int ncols, img::byte * buffer){
    if ((nrows == 0) || (ncols == 0)){
        rows = cols = 0;
        img = 0;
    }
...
LoadResult Image::LoadFromPGM(const char * file_path){
    ...
    img::byte * buffer = ReadPGMImage(file_path, rows, cols);
    if (!buffer)
...
Image::Image (int nrows, int ncols, img::byte value){
...
void Image::set_pixel (int i, int j, img::byte value) {
    img[i][j] = value;
}
img::byte Image::get_pixel (int i, int j) const {
    return img[i][j];
}

// This doesn't work if representation changes
void Image::set_pixel (int k, img::byte value) {
...
// This doesn't work if representation changes
img::byte Image::get_pixel (int k) const {
    // TODO this makes assumptions about the internal representation
